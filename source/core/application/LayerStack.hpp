...
namespace kst::core::application {
  /**
   * @class LayerStack
   * @brief Manages a stack of Layer objects with separate regions for regular layers and overlays
   * 
   * The LayerStack maintains a single vector with two logical regions:
   * 1. Regular layers: Added to the beginning of the stack (bottom of the rendering order)
   * 2. Overlays: Added to the end of the stack (top of the rendering order)
   * 
   * This design allows for efficient iteration through all layers while maintaining
   * the appropriate rendering and event handling order. Regular layers are rendered first,
   * then overlays. Events propagate from overlays down to regular layers.
   */
  class LayerStack {
  public:
    LayerStack() = default;
