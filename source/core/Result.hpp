...
namespace kst::core {
  ...
  /**
   *  @class Result
   *  @brief A generic container for operation results that can either succeed with a value or fail
   *  with and error
   *
  ...
  public:
    ...
    auto valueOr(const T& defaultValue) const -> T { return m_value.value_or(defaultValue); }
    ...
    auto onError(std::function<void(const T&)> func) -> Result<T>& {
      if (hasError()) {
        func(m_error);
      }
      return *this;
  ...
  public:
    ...
    template <typename Fn>
    auto andThen(Fn&& func) const -> decltype(func()) {
    ...
    template <typename Fn>
    auto onSuccess(std::function<void()> func) {
      if (m_success) {
        func();
      }
