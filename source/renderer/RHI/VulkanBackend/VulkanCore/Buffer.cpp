...
namespace VulkanCore {
  ...
  Buffer::Buffer(
    ...
    ASSERT(
        actualBufferIfStaging_->usage_ & VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        "Actual buffer must be dst buffer in case of staging buffer"
    );
    ASSERT(
        actualBufferIfStaging_->allocCreateInfo_.usage == VMA_MEMORY_USAGE_GPU_ONLY,
    ...
    VK_CHECK(
        vmaCreateBuffer(allocator_, &createInfo, &allocCreateInfo_, &buffer_, &allocation_, nullptr)
    );
  ...
  Buffer::Buffer(
      ...
      : context_{context}, allocator_(vmaAllocator), size_(createInfo.size),
        usage_(createInfo.usage), allocCreateInfo_(allocInfo) {
  ...
  void Buffer::uploadStagingBufferToGPU(
    ...
    ASSERT(
        actualBufferIfStaging_ != nullptr,
