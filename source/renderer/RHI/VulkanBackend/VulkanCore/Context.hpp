...
namespace VulkanCore {
  ...
  public:
    ...
    std::shared_ptr<Buffer> createPersistentBuffer(
        size_t size,
        VkBufferUsageFlags flags,
        const std::string& name = ""
    ) const;
    ...
    CommandQueueManager createGraphicsCommandQueue(
        uint32_t count,
    ...
    CommandQueueManager createTransferCommandQueue(
        uint32_t count,
        uint32_t concurrentNumCommands,
        const std::string& name,
  ...
  private:
    ...
    enumerateInstanceLayers(bool printEnumerations_ = false);
