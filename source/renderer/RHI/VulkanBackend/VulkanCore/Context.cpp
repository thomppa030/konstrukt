...
namespace {
  ...
  VkBool32 VKAPI_PTR debugMessengerCallback(
    ...
    if (messageSeverity & (VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)) {
      LOGE(
          "debugMessengerCallback : MessageCode is %s & Message is %s",
...
// Surface
    ...
    // When using GLFW, we can use its cross-platform surface creation
    if (window != nullptr) {
      VK_CHECK(glfwCreateWindowSurface(instance_, static_cast<GLFWwindow*>(window), nullptr, &surface_));
    }
#elif defined(VK_USE_PLATFORM_WIN32_KHR) && defined(VK_KHR_win32_surface)
    ...
    if (physicalDevice_.computeFamilyIndex().has_value()) {
      if (physicalDevice_.computeFamilyCount() > 0) {
        ...
        for (int i = 0; i < computeQueues_.size(); ++i) {
          vkGetDeviceQueue(
              device_,
              physicalDevice_.computeFamilyIndex().value(),
              uint32_t(i),
              &computeQueues_[i]
          );
        }
    ...
    if (physicalDevice_.sparseFamilyIndex().has_value()) {
      if (physicalDevice_.sparseFamilyCount() > 0) {
        ...
        for (int i = 0; i < sparseQueues_.size(); ++i) {
          vkGetDeviceQueue(
              device_,
              physicalDevice_.sparseFamilyIndex().value(),
              uint32_t(i),
              &sparseQueues_[i]
          );
        }
    ...
    if (physicalDevice_.presentationFamilyIndex().has_value()) {
      vkGetDeviceQueue(
          device_,
          physicalDevice_.presentationFamilyIndex().value(),
          0,
          &presentationQueue_
      );
    }
...
#  endif
  ...
  std::shared_ptr<Buffer> Context::createPersistentBuffer(
      size_t size,
      VkBufferUsageFlags flags,
      const std::string& name
  ) const {
  ...
  std::shared_ptr<Buffer> Context::createStagingBuffer(
      VkDeviceSize size,
      VkBufferUsageFlags flags,
      const std::string& name
  ) const {
  ...
  void Context::uploadToGPUBuffer(
      VulkanCore::CommandQueueManager& queueMgr,
      VkCommandBuffer commandBuffer,
      VulkanCore::Buffer* gpuBuffer,
      const void* data,
      long totalSize,
      uint64_t gpuBufferOffset
  ) const {
  ...
  CommandQueueManager Context::createGraphicsCommandQueue(
      uint32_t count,
    ...
    if (graphicsQueueIndex != -1) {
      ASSERT(
          graphicsQueueIndex < graphicsQueues_.size(),
  ...
  VulkanCore::CommandQueueManager Context::createTransferCommandQueue(
    ...
    if (transferQueueIndex != -1) {
      ASSERT(
          transferQueueIndex < transferQueues_.size(),
  ...
  std::shared_ptr<ShaderModule> Context::createShaderModule(
  ...
  std::shared_ptr<Pipeline> Context::createGraphicsPipeline(
      const Pipeline::GraphicsPipelineDescriptor& desc,
  ...
  void Context::beginDebugUtilsLabel(
      VkCommandBuffer commandBuffer,
      const std::string& name,
